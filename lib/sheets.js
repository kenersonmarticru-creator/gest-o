...
class SheetsService {
  ...
  async salvarNaBase(dados) {
    try {
      ...
      // chave identifica de quem são os registros: usa aba se disponível, senão supervisor
      // garante que cada usuário só apaga/substitui os seus próprios dados do dia
      const chave = abaChave || supervisorChave;
      ...
      // Usa a mesma lógica chave = aba || supervisor para identificar a linha:
      ...
      const rowsParaRemover = rowsExistentes.filter(row => {
        ...
        const rowAba  = String(row.get('Aba')        || '').trim();
        const rowData = String(row.get('Data')       || '').trim();
        const rowChave = rowAba || rowSup;
        return rowChave === chave && rowData === hoje;
      });

      console.log(`[SHEETS] Removendo ${rowsParaRemover.length} registros antigos do dia (chave="${chave}")`);
      ...
      // Adiciona em lote — INSERT_ROWS garante que as linhas são acrescentadas
      // ao final da planilha sem sobrescrever dados de outros usuários
  ...
  async processarBaseColado(supervisor, aba, dadosColados) {
    try {
      ...
      // Remove apenas os registros deste usuário (chave+data) — mesma lógica do salvarNaBase
      ...
      await sheet.addRows(linhasProcessadas, { insert: true });
